...
# Join the data_geral, data_ativo_passivo and complemento_data tables

dados_crus <- inner_join(data_geral, data_ativo_passivo, by = c("CNPJ_Fundo", "Data_Referencia")) %>% 
    inner_join(complemento_data, by = c("CNPJ_Fundo", "Data_Referencia"))
    
    
    # %>%
    # left_join(complemento_data, by = "CNPJ_Fundo")
...
codigos_filtro <- n_distinct(dados_crus$Codigo_ISIN)
...
## Fundos de investimento imobiliário listados na B3
```{r}
#| echo: true
#| eval: false
...
# Create a vector of unique ticker codes

dados_crus <- dados_crus %>%
    filter(!is.na(Codigo_ISIN))

data_geral_filtrado <- dados_crus %>%
    mutate(codigos_ticker = sapply(Codigo_ISIN, function(x) {
        detected_code <- codigos[str_detect(x, codigos)]
        if(length(detected_code) > 0) {
            return(detected_code)
        } else {
            return(NA)
        }
    })) %>%
    filter(!is.na(codigos_ticker))

# Save the filtered data to a csv file

write_csv(data_geral_filtrado, "dados/data_geral_filtrado_b3.csv")
...
A seguir vamos filtrar apenas Fundos que investem em ativos de tijolo. Vamos usar todos os tipos menos NA e "Títulos e Val. Mob."

O Segundo filtro é o de numero de cotistas, vamos usar apenas fundos abertos para investidores em geral e que possuam mais de 50 cotistas.

```{r}

#filtro de segmento de atuação

data_geral_filtrado <- data_geral_filtrado %>%
    filter(Segmento_Atuacao != "Títulos e Val. Mob." & !is.na(Segmento_Atuacao))

#filtro de numero de cotistas

data_geral_filtrado <- data_geral_filtrado %>%
    filter(Total_Numero_Cotistas >= 50)


# # CNPJ UNICOS

# cnpj_unicos <- n_distinct(dados_crus$CNPJ_Fundo) %>% 
#     print()
