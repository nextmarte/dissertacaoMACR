...
download_and_read <- function(year) {
    ...
    data_ativo_passivo <- tryCatch(read_delim(unz(zip_file,
    ...
    data_complemento <- tryCatch(read_delim(unz(zip_file, paste0("inf_mensal_fii_complemento_", year, ".csv")), delim = ";", locale = locale(encoding = "ISO-8859-1"), show_col_types = FALSE), error = function(e) {
        message("Error reading complemento data for year ", year, ": ", e)
        return(NULL)
    })
    data_geral <- tryCatch(read_delim(unz(zip_file, paste0("inf_mensal_fii_geral_", year, ".csv")), delim = ";", locale = locale(encoding = "ISO-8859-1"), show_col_types = FALSE), error = function(e) {
        message("Error reading geral data for year ", year, ": ", e)
        return(NULL)
    })
...
# Use purrr::map instead of a for loop
data_list <- map(years, download_and_read)

# Add data to the list
data_list <- map(data_list, function(data) {
    list(
        ativo_passivo = if (!is.null(data$data_ativo_passivo)) data$data_ativo_passivo else NULL,
        geral = if (!is.null(data$data_geral)) data$data_geral else NULL,
        complemento = if (!is.null(data$data_complemento)) data$data_complemento else NULL
    )
})
